...
@section('content')
    ...
    @slot('title') Tableau de bord MINDDEVEL @endslot
    @endcomponent
            ...
            <div class="row">
                <div class="col-lg-2">
                    <div class="nav nav-pills flex-column nav-pills-tab custom-verti-nav-pills text-center"
                        ...
                        @for($i = 0; $i < count($personnels["structure"]); $i++)
                            @if($i==0)
                                <a class="nav-link active show" id="{{Str::remove([" ", "'", "-"], $personnels["structure"][$i])."-tab"}}" data-bs-toggle="pill"
                                   href="#{{Str::remove([" ", "'", "-"], $personnels["structure"][$i])}}" role="tab" aria-controls="{{Str::remove([" ", "'", "-"], $personnels["structure"][$i])}}"
                                   aria-selected="true">
                                    {{$personnels["structure"][$i]}}</a>
                            @else
                                <a class="nav-link" id="{{Str::remove([" ", "'", "-"], $personnels["structure"][$i])."-tab"}}" data-bs-toggle="pill"
                                   href="#{{Str::remove([" ", "'", "-"], $personnels["structure"][$i])}}" role="tab" aria-controls="{{Str::remove([" ", "'", "-"], $personnels["structure"][$i])}}"
                                   aria-selected="false">
                                    {{$personnels["structure"][$i]}}</a>
                            @endif
                ...
                <div class="col-lg-10">
                    <div class="tab-content text-muted mt-3 mt-lg-0">
                        ...
                        @for($i = 0; $i < count($personnels["structure"]); $i++)
                            @if($i==0)
                                <div class="tab-pane fade active show" id="{{Str::remove([" ", "'", "-"], $personnels["structure"][$i])}}" role="tabpanel"
                                     aria-labelledby="{{ Str::remove([" ", "'", "-"], $personnels["structure"][$i])."-tab" }}">
                                    @else
                                        <div class="tab-pane fade" id="{{Str::remove([" ", "'", "-"], $personnels["structure"][$i])}}" role="tabpanel"
                                             aria-labelledby="{{ Str::remove([" ", "'", "-"], $personnels["structure"][$i])."-tab" }}">
                                            @endif
                                            <div class="table-responsive table-card">
                                                <table class="table table-nowrap table-striped-columns mb-0">
                                                    ...
                                                    <tbody>
                                                    @for($j = 0; $j < count($personnels["personnel"]); $j++)
                                                        @if($personnels["personnel"][$j]["structure"] == $personnels["structure"][$i])
                                                            <tr>
                                                                <td></td>
                                                                <td>
                                                                    <a href="#">
                                                                        {{$personnels["personnel"][$j]["nom"]." ".$personnels["personnel"][$j]["prenom"]}}
                                                                    </a>
                                                                </td>
                                                                @if($personnels["personnel"][$j]["sexe"] == "Male")
                                                                    <td>{{__("Masculin")}}</td>
                                                                @elseif($personnels["personnel"][$j]["sexe"] == "Female")
                                                                    <td>{{__("Feminin")}}</td>
                                                                @endif
                                                                <td>{{$personnels["personnel"][$j]["poste"]}}</td>
                                                                <td>{{$personnels["personnel"][$j]["structure"]}}</td>
                                                            </tr>
                                                        @endif
                                                    @endfor
                                                    </tbody>
